...
Timeline:
  Comeca: 23:35 UTC (HOJE)
  Termina: 24/02 06:00 UTC (AMANHA CAFE)
  Duracao: 2-3 horas
  Criticidade: CRITICO (bloqueia Grid Search)

Acceptance Criteria:
  [OK] load_and_label() implementada
  [OK] Zero NaN values
  [OK] Imbalance < 70%
  [OK] Unit tests 100% passing
  [OK] Code review aprovado
  [OK] Performance < 500ms
"""
...
class BacktestLabeler:
    ...
    def load_backtest_results(self) -> Dict[str, Any]:
        ...
        if not self.backtest_file.exists():
            raise FileNotFoundError(f"Arquivo no encontrado: {self.backtest_file}")
    ...
    def create_labels(self) -> np.ndarray:
        ...
        Lgica:
          ...
          - Se no tem signal ou falso alarme: label = 0 (skip)
        """
        ...
        if imbalance_pct > 70:
            self.logger.warning(f" ATENO: Imbalance acima de 70%: {imbalance_pct:.1f}%")
        else:
    ...
    def validate_labels(self) -> bool:
        """Validar labels: zero NaN, imbalance aceitvel."""
        ...
        if self.labels.dtype == np.int32:
            ...
            self.logger.warning(f"   Tipo: {self.labels.dtype} (esperado int32)")
        
        # Check 3: Valores vlidos (0 ou 1)
        valid_values = np.all(np.isin(self.labels, [0, 1]))
        if valid_values:
            self.logger.info("   Todos valores so 0 ou 1")
            checks_passed += 1
        else:
            self.logger.error("   Encontrados valores invlidos (no 0 ou 1)")
        ...
        if imbalance < 70:
            ...
            self.logger.warning(f"   Imbalance alto: {imbalance:.1f}%")
        
        # Check 5: Tamanho positivo
        if len(self.labels) > 0:
            self.logger.info(f"   Tamanho vlido: {len(self.labels)} labels")
            checks_passed += 1
        else:
            self.logger.error("   Array de labels vazio")
        
        self.logger.info(f" Validao: {checks_passed}/{checks_total} checks passed")
    ...
    def prepare_dataset(self) -> Tuple[pd.DataFrame, np.ndarray]:
        ...
        Carrega features de feature_pipeline (j computadas)
        Associa com labels recm-criados
        """
    ...
    def execute(self) -> bool:
        """
        Executar pipeline completo de labelao.
        
        Returns: True se sucesso, False se falhou
        """
        try:
            self.logger.info("=" * 60)
            self.logger.info(" TODO-1: Iniciando labelao de backtest results")
            self.logger.info("=" * 60)
            ...
            if not self.validate_labels():
                self.logger.error(" Validao de labels falhou!")
                return False
            ...
            self.logger.info("=" * 60)
            self.logger.info("\nProximas aes:")
            self.logger.info("  | Grid Search pode comear (07:00 BRT)")
            self.logger.info("  | Notificar Eng Sr que labels prontos para E2E")
            
            return True
            
        except Exception as e:
            self.logger.error(f" ERRO durante execuo: {e}", exc_info=True)
            return False


def main():
    """Funo principal para executar TODO-1."""
    print("""
[===========================================================]
|    TODO-1: LABEL BACKTEST OPTIMIZED RESULTS             |
|       Owner: ML Expert                                     |
|       Timeline: 23:35 UTC - 06:00 UTC (+1 dia)            |
|       Criticidade: CRTICO (bloqueia Grid Search)          |
[===========================================================
    """)
