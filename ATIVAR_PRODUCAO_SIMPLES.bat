...
start "BDI Detector - Productino" cmd /k "cd /d "%~dp0" && python -m src.application.services.processador_bdi --config config\producao_simples.yaml && pause"

timeout /t 2 /nobreak

REM Terminal 2: Risk Validator
echo [Terminal 2] Risk Validator...
start "Risk Validator - Production" cmd /k "cd /d "%~dp0" && python -m src.application.risk_validator --config config\producao_simples.yaml && pause"

timeout /t 2 /nobreak

REM Terminal 3: Orders Executor
echo [Terminal 3] Orders Executor...
start "Orders Executor - Production" cmd /k "cd /d "%~dp0" && python -m src.application.orders_executor --config config\producao_simples.yaml && pause"
